...
class DQN:
    def __init__(self, model, num_actions, gamma, max_experiences, min_experiences, batch_size, lr):
...
def main():
    ...
    conv = tf.keras.layers.Conv2D(100, (3, 3), activation='relu', kernel_initializer='RandomNormal')(inputs)
    gmax = tf.keras.layers.GlobalMaxPool2D()(conv)
    flatten1 = tf.keras.layers.Flatten()(gmax)

    flatten2 = tf.keras.layers.Flatten()(inputs)
    dense1 = tf.keras.layers.Dense(50, activation='relu', kernel_initializer='RandomNormal')(flatten2)
    dense2 = tf.keras.layers.Dense(50, activation='relu', kernel_initializer='RandomNormal')(dense1)

    conc = tf.keras.layers.Concatenate()([flatten1, dense1])
    dense2 = tf.keras.layers.Dense(num_actions, activation='tanh', kernel_initializer='RandomNormal')(conc)

    model = tf.keras.Model(inputs=inputs, outputs=dense2, name='on_test')
    model.summary()

    print("Creation of DQN model :")
    TrainNet = DQN(model, num_actions, gamma, max_experiences, min_experiences, batch_size, lr)
    TargetNet = DQN(model, num_actions, gamma, max_experiences, min_experiences, batch_size, lr)
    ...
    for n in range(N):
        ...
        if (n+1) % N_info == 0 or n == 0:
            ...
            template = "Successfull episode: {0:7d}/{1:7d} | eps: {2:5.2f}% | reward: {3:5.0f} (avg) | loss: {4:6.0f} (avg) | time: {5:.3f} s (avg) | time total: {6:.0f}"
